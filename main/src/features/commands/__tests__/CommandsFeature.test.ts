...
vi.mock('../handlers/InfoCommandHandler', () => ({ InfoCommandHandler: vi.fn().mockImplementation(function () { return mockHandler }) }))
vi.mock('../handlers/HelpCommandHandler', () => ({ HelpCommandHandler: vi.fn().mockImplementation(function () { return mockHandler }) }))
vi.mock('../handlers/StatusCommandHandler', () => ({ StatusCommandHandler: vi.fn().mockImplementation(function () { return mockHandler }) }))
vi.mock('../handlers/BindCommandHandler', () => ({ BindCommandHandler: vi.fn().mockImplementation(function () { return mockHandler }) }))
...
describe('CommandsFeature', () => {
    ...
    beforeEach(async () => {
        ...
        // Mock all handlers
        const mockHandler = { execute: vi.fn() }
        const handlerMock = vi.fn().mockImplementation(function () { return mockHandler })
        vi.doMock('../handlers/InfoCommandHandler', () => ({ InfoCommandHandler: handlerMock }))
        vi.doMock('../handlers/HelpCommandHandler', () => ({ HelpCommandHandler: handlerMock }))
        vi.doMock('../handlers/StatusCommandHandler', () => ({ StatusCommandHandler: handlerMock }))
        vi.doMock('../handlers/BindCommandHandler', () => ({ BindCommandHandler: handlerMock }))
        ...
        vi.doMock('../../../plugins/core/event-publisher', () => ({
            getEventPublisher: vi.fn().mockReturnValue({
                publishMessage: vi.fn(),
            }),
        }))
    ...
    it('check for initialization errors', async () => {
        ...
        const logger = (await import('../../../shared/logger')).getLogger('CommandsFeature')
