...
export async function initWeb(app: ReturnType<typeof createServer>, options: BootstrapOptions) {
  if (options.startWeb !== false) {
    await startServer(app)
  }
}

export async function initInstances(targets: number[], activeInstances: Map<number, Instance>, log: ReturnType<typeof getLogger>) {
  ...
  return { succeeded, failed, targetCount: targets.length }
}

export async function bootstrap(options: BootstrapOptions = {}) {
  const log = getLogger('Main')
  const app = createServer()
  configureRuntimeBridge(app, {
    getInstance: instanceId => instanceRegistry.getById(instanceId) as any,
    listInstances: () => instanceRegistry.getAll() as any[],
    getRuntimeReport: () => {
      try {
        return PluginRuntime.getLastReport()
      }
      catch {
        return null
      }
  ...
  await initInfra(log)
  const targets = await initRuntime()
  await initWeb(app, options)
  const { succeeded, failed, targetCount } = await initInstances(targets, activeInstances, log)
  ...
  log.info(`启动完成 (instances=${targetCount}, succeeded=${succeeded.length}, failed=${failed.length})`)
}
