...
export function parseChannelId(raw) {
    ...
    if (parts.length >= 2 && (parts[0] === 'qq' || parts[0] === 'tg' || parts[0] === 'telegram')) {
        const platform = parts[0] === 'qq' ? 'qq' : 'tg';
        if (platform === 'qq') {
...
export function parseReplyToForPlatform(replyToRaw, platform) {
...
export function pluginSegmentsToUnifiedContents(segments) {
    const out = [];
...
export class MessageAPIImpl {
     ...
     * 通过实例发送消息（Phase 4 实现）
     */
    async sendViaInstance(instance, params) {
     ...
     * 通过实例撤回消息（Phase 4 实现）
     */
    async recallViaInstance(instance, messageId) {
     ...
     * 通过实例获取消息（Phase 4 实现）
     */
    async getViaInstance(instance, messageId) {
        const parsed = parseMessageId(messageId);
        if (parsed.platform !== 'qq') {
        ...
        if (!msg)
            return null;
        const segments = (msg.content || []).map((c) => {
            if (!c)
 ...
 * 创建消息 API 实例
 */
export function createMessageAPI(instanceResolver) {
