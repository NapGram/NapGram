...
export class PluginRuntime {
    static webRoutes;
    static setWebRoutes(register) {
        this.webRoutes = register;
    }
    static async reloadCommandsForInstances() {
        for (const instance of Instance.instances) {
            try {
                const featureManager = instance?.featureManager;
    ...
    static configureApis() {
        const instanceResolver = (instanceId) => Instance.instances.find(it => it.id === instanceId);
     ...
     * 重载单个插件（不重启整个运行时）
     */
    static async reloadPlugin(pluginId) {
