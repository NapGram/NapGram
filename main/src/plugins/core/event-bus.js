...
export class EventBus {
     ...
     * @returns 事件订阅句柄
     */
    subscribe(eventType, handler, filter, pluginId) {
     ...
     * @returns 事件订阅句柄
     */
    once(eventType, handler, filter, pluginId) {
        const subscription = this.subscribe(eventType, async (event) => {
            await handler(event);
    ...
    async publish(eventType, event) {
        ...
        for (const sub of subs) {
            ...
            // 执行处理器（包装错误处理）
            const promise = this.executeHandler(sub.config.handler, event, eventType, sub.config.pluginId);
     ...
     * @param pluginId 插件 ID
     */
    async executeHandler(handler, event, eventType, pluginId) {
        try {
            await handler(event);
            this.stats.handled++;
        }
            ...
            logger.error({ error, eventType, pluginId }, `Event handler error ${context}: ${eventType}`);
            // 错误不会传播，避免影响其他订阅者
        }
    }
    /**
     ...
     * @returns 订阅数
     */
    getPluginSubscriptionCount(pluginId) {
        let count = 0;
        for (const subs of this.subscriptions.values()) {
            for (const sub of subs) {
                if (sub.config.pluginId === pluginId) {
