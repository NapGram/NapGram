...
export function findBoundQQGroup(event) {
    ...
    if (!pair) {
        ...
        const pairInfo = allPairs.slice(0, 5).map((p) => `[TG:${p.tgChatId}:${p.tgThreadId}]`).join(', ');
...
export async function getGroupHonor(event, args) {
    ...
    try {
        ...
        if (honorInfo.talkative_list && honorInfo.talkative_list.length > 0) {
            message += `💬 **历史龙王**\n`;
            honorInfo.talkative_list.slice(0, 3).forEach((user) => {
                message += `• [${user.nickname}](${getLink(user.user_id)}) - ${user.description}\n`;
            });
            message += `\n`;
        }
        ...
        if (honorInfo.performer_list && honorInfo.performer_list.length > 0) {
            message += `🔥 **群聊之火**\n`;
            honorInfo.performer_list.slice(0, 3).forEach((user) => {
                message += `• [${user.nickname}](${getLink(user.user_id)}) - ${user.description}\n`;
            });
            message += `\n`;
        }
        ...
        if (honorInfo.legend_list && honorInfo.legend_list.length > 0) {
            message += `💪 **群霸**\n`;
            honorInfo.legend_list.slice(0, 3).forEach((user) => {
                message += `• [${user.nickname}](${getLink(user.user_id)}) - ${user.description}\n`;
            });
            message += `\n`;
        }
        ...
        if (honorInfo.strong_newbie_list && honorInfo.strong_newbie_list.length > 0) {
            message += `✨ **冒泡新人**\n`;
            honorInfo.strong_newbie_list.slice(0, 3).forEach((user) => {
                message += `• [${user.nickname}](${getLink(user.user_id)}) - ${user.description}\n`;
            });
            message += `\n`;
        }
        ...
        if (honorInfo.emotion_list && honorInfo.emotion_list.length > 0) {
            message += `😂 **快乐源泉**\n`;
            honorInfo.emotion_list.slice(0, 3).forEach((user) => {
                message += `• [${user.nickname}](${getLink(user.user_id)}) - ${user.description}\n`;
            });
            message += `\n`;
        }
