...
function hasForwardPairMetadata(metadata: ForwardPairMetadata): boolean {
...
export async function findPairByQQWithChatType(
  ...
  const fallback = typeof (forwardMap as any).findByQQ === 'function'
    ? forwardMap.findByQQ(qqRoomId)
    : undefined
...
export async function addForwardPairWithChatType(
  ...
  try {
    const rec = chatType === 'private' || hasForwardPairMetadata(metadata)
      ? await forwardMap.add({
          qqChatType: chatType,
          qqRoomId,
          tgChatId,
          tgThreadId: tgThreadId ?? null,
          ...metadata,
        })
      : await forwardMap.add(qqRoomId, tgChatId, tgThreadId)

    if (!rec)
      return undefined
    return attachChatType(rec, chatType)
  }
  catch (error) {
    if (isMissingPersonalModeColumn(error)) {
      if (chatType === 'group') {
