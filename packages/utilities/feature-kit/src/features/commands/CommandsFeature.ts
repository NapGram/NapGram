...
import { RequestManagementCommandHandler } from './handlers/RequestManagementCommandHandler.js'
...
export class CommandsFeature {
  ...
  private async registerDefaultCommands() {
    ...
    this.registerCommand({
      ...
      usage: '/bindfriend <qq_uin> [thread_id]',
      permission: { level: 1 }, // ADMIN
    ...
    this.registerCommand({
      name: 'rejectall',
      aliases: ['批量拒绝'],
      description: '批量拒绝待处理申请',
      usage: '/rejectall [friend|group] [理由]',
      permission: { level: 1 }, // ADMIN
