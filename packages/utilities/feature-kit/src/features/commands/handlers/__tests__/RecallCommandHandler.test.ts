...
function createMessage(platform: 'telegram' | 'qq' = 'telegram', raw?: any): UnifiedMessage {
  return {
    ...
    chat: {
      id: '777777',
      type: platform === 'qq' ? 'private' : 'group',
    },
    content: [
      {
        type: 'text',
...
describe('recallCommandHandler', () => {
  ...
  it('uses QQ chat type when finding a QQ-side recalled source', async () => {
    vi.mocked(mockContext.permissionChecker.isAdmin).mockReturnValue(true)
    ...
    msg.content.push({
      type: 'reply',
      data: { messageId: '42', senderId: '111111', senderName: 'Target' },
    } as any)
    await handler.execute(msg, [])

    expect(vi.mocked((await import('@napgram/infra-kit')).eq)).toHaveBeenCalledWith('qqChatType', 'private')
  })
