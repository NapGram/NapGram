...
export class UnbindCommandHandler {
  ...
  async execute(msg: UnifiedMessage, args: string[]): Promise<void> {
    ...
    const qqTargetId = explicitType ? args[1] : args.length === 1 ? args[0] : undefined
    ...
    const removeTarget = target.qqChatType === 'private'
      ? { type: 'private' as const, id: target.qqRoomId }
      : target.qqRoomId.toString()
    const removed = await forwardMap.remove(removeTarget)
    if (removed === false)
