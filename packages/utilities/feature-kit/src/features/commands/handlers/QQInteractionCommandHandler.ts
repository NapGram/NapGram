...
export class QQInteractionCommandHandler {
  ...
  private async handlePoke(msg: UnifiedMessage, args: string[]) {
    const pair = await this.getGroupPair(msg)
    if (!pair) {
      return
    }
  ...
  private async getGroupPair(msg: UnifiedMessage) {
    if (msg.platform !== 'telegram')
      return null
    const forwardMap = this.context.instance.forwardPairs as ForwardMap
    const threadId = this.context.extractThreadId(msg, [])
    const pair = await findPairByTGWithChatType(forwardMap, msg.chat.id, threadId, Boolean(threadId))
    if (!pair) {
      await this.reply(msg, '未绑定任何 QQ 群')
      return null
    }
    if (pair.qqChatType !== 'group') {
      await this.reply(msg, '当前聊天绑定的是 QQ 好友，QQ 群交互指令不可用')
      return null
    }
    return pair
  }
