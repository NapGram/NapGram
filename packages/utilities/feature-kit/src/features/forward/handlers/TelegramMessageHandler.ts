...
export class TelegramMessageHandler {
  ...
  private async sendForwardSegments(pair: any, node: any, fallbackMsg: UnifiedMessage) {
    ...
    if (typeof this.qqClient.sendPrivateForwardMessage === 'function') {
      const result = await this.qqClient.sendPrivateForwardMessage({
        user_id: qqRoomId,
        messages: [node],
      })
      ...
      if (hasMedia) {
        ...
        if (hasContentToSend) {
          ...
          const headerMsg: UnifiedMessage = {
            ...this.createQqMessage(unified, pair, headerSegments),
          }
        ...
        if (msgId) {
          ...
          try {
            ...
            await db.insert(schema.message).values({
              qqChatType: this.getQqChatType(pair),
