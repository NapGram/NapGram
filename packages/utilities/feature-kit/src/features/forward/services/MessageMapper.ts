...
export class ForwardMapper {
  ...
  async saveMessage(qqMsg: UnifiedMessage, tgMsg: any, instanceId: number, qqRoomId: bigint, tgChatId: bigint) {
    ...
    try {
      ...
      await db.insert(schema.message).values({
        qqChatType: qqMsg.chat?.type === 'private' ? 'private' : 'group',
        qqRoomId,
        qqSenderId,
        time,
        seq,
  ...
  async findTgMsgId(instanceId: number, qqRoomId: bigint, qqMsgId: string, qqChatType: 'private' | 'group' = 'group'): Promise<bigint | undefined> {
