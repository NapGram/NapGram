...
function createQQMessage(overrides: Partial<UnifiedMessage> = {}): UnifiedMessage {
  return {
    ...
    chat: { id: '20002', type: 'private', name: 'Fallback Friend' },
...
describe('PersonalPairProvisioner', () => {
  ...
  it('does nothing when personal auto provisioning is unavailable', async () => {
    ...
    const pair = await provisioner.ensurePairForQQMessage(createQQMessage(), 'private')
  ...
  it('creates a TG supergroup, invites the bot, and stores a typed auto-created friend pair', async () => {
    const runtime = createRuntime()
    const provisioner = new PersonalPairProvisioner(runtime.instance, runtime.forwardMap, runtime.qqClient)

    const pair = await provisioner.ensurePairForQQMessage(createQQMessage(), 'private')

    expect(runtime.qqClient.getFriendInfo).toHaveBeenCalledWith('20002')
    expect(runtime.tgUserClient.createSupergroup).toHaveBeenCalledWith({
      title: 'QQ 好友 Alice',
      description: 'NapGram personal mode auto-created for QQ 好友 20002',
      forum: false,
    })
  ...
  it('reuses one provisioning task for concurrent messages from the same QQ chat', async () => {
    ...
    const first = provisioner.ensurePairForQQMessage(msg, 'private')
    const second = provisioner.ensurePairForQQMessage(msg, 'private')
