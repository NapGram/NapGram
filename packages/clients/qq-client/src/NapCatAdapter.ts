...
export class NapCatAdapter extends EventEmitter {
  ...
  constructor(private readonly params: NapCatCreateParams) {
    ...
    super()
    const reconnectOpt = params.reconnect
    let reconnectCfg: { enabled: boolean, maxAttempts?: number, backoff?: { initial: number, max: number, multiplier: number } } | undefined

    if (reconnectOpt === false) {
      reconnectCfg = { enabled: false, maxAttempts: 0 }
    }
    else if (reconnectOpt === undefined) {
      reconnectCfg = undefined
    }
    else if (reconnectOpt === true) {
      reconnectCfg = { enabled: true }
    }
    else {
      const maxAttempts = reconnectOpt.maxAttempts ?? 3
      const interval = reconnectOpt.interval ?? 5000
      reconnectCfg = {
        enabled: true,
        maxAttempts,
        backoff: { initial: interval, max: interval, multiplier: 1 },
      }
    }
